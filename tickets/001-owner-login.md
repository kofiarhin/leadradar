...
- Re-verified on 2026-09-11 under `tickets/003-stabilize-foundation.md` at `fix/003-stabilize-foundation` (Node 22.22.2): `npm test` (66 server + 16 client), `npm run typecheck`, `npm run lint`, and `npm run build` all pass. HTTP-level runtime verification against the built server and an in-memory MongoDB passed 37/37, covering unauthenticated 401s on `/workspace`, `/vertical-profile` and `/auth/session`, wrong password → 401 `AUTH_INVALID_CREDENTIALS`, unknown account → identical 401 message, malformed login body → 400, valid login → 200 with an `HttpOnly; SameSite=Lax` session cookie, authenticated session and workspace reads, `POST /auth/logout` → 204, and all protected reads → 401 with the pre-logout cookie afterwards. No password or session secret appeared in seed output, responses, or server logs.
- The browser pass remains outstanding after that re-verification: browser automation is unavailable in this environment and no browser-testing dependency was added. The two unchecked acceptance criteria still stand.
